(() => {
  ...
  function installCalculator() {
    ...
    style.textContent = `
      ...
      .game-calculator-resume{display:none;z-index:10001;background:#2764e7}.game-calculator-overlay.open .game-calculator-resume{display:grid}
      body.game-calculator-is-open>.game-calculator-trigger{display:none}
      @media(max-width:620px){.game-calculator-page{width:min(100% - 20px,760px);padding:18px 0 88px;justify-content:flex-start}.game-calculator-brand{margin-bottom:14px}.game-calculator-body{padding:11px}.game-calculator-display{min-height:100px}.game-calculator-keys{gap:6px}.game-calculator-key{min-height:44px;font-size:13px}.game-calculator-key.function{font-size:10px}}
    ...
    overlay.innerHTML = `
      <main class="game-calculator-page">
        ...
        <button class="game-calculator-resume" type="button" aria-label="Return to game">${joyconIcon}</button>
      </main>`;
